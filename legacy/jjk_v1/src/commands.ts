...
States:
  ...
  jjk see [--deleted] [--table] [-v2] [-v3] [-v4]
  jjk graph [--deleted]
  jjk show [state]
  jjk story
...
export async function runCli(argv: string[], cwd: string): Promise<void> {
  ...
  switch (command) {
    ...
    case "see": {
      ...
      const useV2Graph = args.includes("-v2") || args.includes("--v2");
      const useV3Graph = args.includes("-v3") || args.includes("--v3");
      const useV4Graph = args.includes("-v4") || args.includes("--v4");
      const currentState = resolveCurrentState(root, repo.states);
      if (showTable) {
        console.log(
      ...
      console.log(renderGraph(repo, {
        ...
        colorize,
        includeDeleted,
        compactSameBranch: useV2Graph && !useV3Graph && !useV4Graph,
        branchAlignedV3: useV3Graph && !useV4Graph,
        trimOneIndentOnSameBranch: useV4Graph,
      }));
      return;
    }
    case "graph": {
