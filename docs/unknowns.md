...
SQLite with one database in the repository’s Git common directory is the best v0.1 default because it provides schema constraints, bounded indexed queries, transactional event+projection updates, online backup, and one shared control plane across linked worktrees. Enable WAL only after a capability probe proves local-filesystem locking and durable sync semantics. On unsupported/network filesystems, allow proven read-only inspection and refuse semantic mutation with `JJK-E-STORAGE-UNSAFE`; v0.1 does not silently activate an unproven rollback-journal fallback.
...
| 15 / `R-15` | UK | UX/adoption | 4×3 | Free-form descriptions or fuzzy return can turn a typo into mutation or pick the wrong similarly named state; an agent prompt can hang automation. | Require explicit `jjk save -- <text>`; exact confidence rules; choices on TTY; structured ambiguity error non-interactively; `--format json --non-interactive`; no implicit editor. | **Blocker** |
...
4. WAL probe fails on SMB, so semantic mutations remain disabled rather than “best effort”; transparent Git and proven read-only inspection remain available.
