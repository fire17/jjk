...
fn discovers_subdirectory_and_shared_common_dir_in_linked_worktree() {
    ...
    git(&primary, &["worktree", "add", "-q", linked.to_str().unwrap(), "-b", "linked"]);
    ...
    assert_eq!(linked_facts.worktree_root.as_deref(), Some(fs::canonicalize(&linked).unwrap().as_path()));
...
fn supervised_passthrough_preserves_exit_code() {
    let fixture = TempDir::new().unwrap();
    let request = passthrough("git", vec![OsString::from("definitely-not-a-git-command")], fixture.path());
...
fn broken_jj_degrades_without_affecting_git() {
    ...
    assert!(matches!(result, JjCapabilities::Degraded { version: Some(_), diagnostic } if diagnostic.contains("broken operation log")));

    let git_result = OsProcess.run_captured(&jjk::ports::process::CapturedProcess {
        executable: "git".into(), args: vec![OsString::from("--version")], cwd: fixture.path().to_path_buf(), env_delta: Default::default(),
    }).unwrap();
