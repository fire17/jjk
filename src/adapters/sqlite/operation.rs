...
use crate::ports::operation::{
    OperationRecord, OperationStatus, OperationStore, PreparedOperationRecord,
};
...
impl OperationStore for SqliteStore {
    ...
    fn operation(&self, operation_id: Uuid) -> Result<Option<OperationRecord>, Self::Error> {
        get(&self.connection, operation_id)
    }
...
fn decode(row: &rusqlite::Row<'_>) -> rusqlite::Result<OperationRecord> {
    ...
    let request: Vec<u8> = row.get(1)?;
    let status: String = row.get(3)?;
    let recovery: Option<Vec<u8>> = row.get(8)?;
    Ok(OperationRecord {
        operation_id: Uuid::from_slice(&id).map_err(|error| convert(id.len(), error))?,
        request_hash: request.try_into().map_err(|value: Vec<u8>| {
            convert(
                value.len(),
                std::io::Error::new(std::io::ErrorKind::InvalidData, "request hash length"),
            )
        })?,
        command_kind: row.get(2)?,
        status: OperationStatus::parse(&status).ok_or(rusqlite::Error::InvalidQuery)?,
        prepared_seq: row.get(4)?,
        terminal_seq: row.get(5)?,
        precondition_fingerprint: row.get(6)?,
        expected_effects: row.get(7)?,
        recovery_artifact_hash: recovery
            .map(|value| {
...
fn convert<E: std::error::Error + Send + Sync + 'static>(
    length: usize,
    error: E,
) -> rusqlite::Error {
    rusqlite::Error::FromSqlConversionFailure(length, rusqlite::types::Type::Blob, Box::new(error))
}
