...
pub(super) fn run(connection: &mut Connection, applied_at_utc: &str) -> Result<(), StoreError> {
    ...
    if has_migrations {
        ...
        match stored {
            ...
            None => return Err(StoreError::InvalidData("schema_migrations exists without initial migration".into())),
        }
    }
...
fn verify_version(connection: &Connection) -> Result<(), StoreError> {
    ...
    if version != STORAGE_SCHEMA_VERSION { return Err(StoreError::UnsupportedSchema(version)); }
    Ok(())
}
