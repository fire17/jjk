...
pub(crate) fn put_record(
    ...
    let actual: u32 = tx.query_row(
        "SELECT reducer_version FROM projection_meta WHERE projection_name = ?1",
        [projection],
        |row| row.get(0),
    )?;
    if actual != reducer_version {
        return Err(StoreError::ReducerVersionMismatch {
            name: projection.into(),
            expected: reducer_version,
            actual,
        });
    }
...
impl ProjectionStore for SqliteStore {
    ...
    fn projection_snapshot(
        ...
        let (actual_version, seq, hash, stored_digest) =
            meta.ok_or_else(|| StoreError::ProjectionStale {
                name: name.into(),
                journal_seq: head.local_seq,
                projection_seq: 0,
            })?;
        if actual_version != reducer_version {
            return Err(StoreError::ReducerVersionMismatch {
                name: name.into(),
                expected: reducer_version,
        ...
        if seq != head.local_seq || projected_hash != head.event_hash {
        ...
        if stored_digest != actual_digest {
            return Err(StoreError::InvalidData(format!(
                "projection {name} digest mismatch"
            )));
        }
        drop(statement);
        tx.commit()?;
...
fn fixed<const N: usize>(bytes: Vec<u8>, field: &str) -> Result<[u8; N], StoreError> {
    bytes.try_into().map_err(|value: Vec<u8>| {
        StoreError::InvalidData(format!("{field} has length {}, expected {N}", value.len()))
    })
}
