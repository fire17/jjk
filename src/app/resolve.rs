...
impl From<&StateReadModel> for ResolutionCandidate {
    fn from(state: &StateReadModel) -> Self {
        Self {
            ...
            attempt: state.attempt_id.to_string(),
            created_at_utc: state.created_at_utc.clone(),
        }
    }
}
    ...
    pub fn require_unique(self) -> Result<ResolutionCandidate, ResolveError> {
        match self {
            ...
            Self::Ambiguous { query, candidates } => {
                Err(ResolveError::Ambiguous { query, candidates })
            }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Ambiguous { query, candidates } => write!(
                f,
                "target {query:?} is ambiguous ({} candidates); use an exact state ID",
                candidates.len()
            ),
...
pub fn resolve_state(
    ...
    let visible = states
        .iter()
        .filter(|state| include_archived || !state.archived)
        .collect::<Vec<_>>();
    ...
    if let Some(state) = visible.iter().find(|state| state.id.to_string() == query) {
        return Ok(Resolution::Exact {
            candidate: ResolutionCandidate::from(*state),
        });
    }
    let exact = visible
        .iter()
        .filter(|state| state.label == query || state.message.as_deref() == Some(query))
        .copied()
        .collect::<Vec<_>>();
    if exact.len() == 1 {
        return Ok(Resolution::Exact {
            candidate: ResolutionCandidate::from(exact[0]),
        });
    }
    if exact.len() > 1 {
        return Ok(Resolution::Ambiguous {
            query: query.to_owned(),
            candidates: canonical_candidates(exact),
        });
    }
        ...
        .filter(|state| {
            state.id.to_string().starts_with(query)
                ...
                || state
                    .message
                    .as_deref()
                    .is_some_and(|message| message.to_lowercase().contains(&folded))
        })
        .copied()
        .collect::<Vec<_>>();
    match matches.len() {
        ...
        1 => Ok(Resolution::UniqueMatch {
            candidate: ResolutionCandidate::from(matches[0]),
        }),
        _ => Ok(Resolution::Ambiguous {
            query: query.to_owned(),
            candidates: canonical_candidates(matches),
        }),
    }
}
