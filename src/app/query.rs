...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct AnnotationReadModel {
    pub kind: String,
    pub value: String,
}
...
impl StateReadModel {
    pub fn is_starred(&self) -> bool {
        self.annotations.iter().any(|a| a.kind == "star")
    }
    pub fn is_trusted(&self) -> bool {
        self.annotations
            .iter()
            .any(|a| a.kind == "trust" && a.value == "trusted")
    }
}
...
impl RepositorySnapshot {
    pub fn canonicalize(&mut self) {
        ...
        for state in &mut self.states {
            state.annotations.sort_by(|left, right| {
                left.kind
                    .cmp(&right.kind)
    ...
    pub fn visible_states(&self, include_archived: bool) -> impl Iterator<Item = &StateReadModel> {
        self.states
            .iter()
            .filter(move |state| include_archived || !state.archived)
    }
    pub fn state(&self, id: StateId) -> Option<&StateReadModel> {
        self.states.iter().find(|state| state.id == id)
    }
    pub fn attempt(&self, id: AttemptId) -> Option<&AttemptReadModel> {
...
pub struct GraphNode {
    ...
    pub markers: BTreeSet<SemanticMarker>,
    pub depth: usize,
    pub lane: usize,
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::StateNotFound(state) => {
                write!(f, "state {state} does not exist at this revision")
            }
            Self::InvalidGraph { state, reason } => {
                write!(f, "invalid graph at state {state}: {reason}")
            }
            Self::PatchUnavailable { from, to, scope } => {
                write!(f, "{scope:?} diff from {from:?} to {to} is unavailable")
            }
        }
    }
}
...
pub struct QueryService<'source, S: ReadSnapshotSource + ?Sized> {
    source: &'source S,
}
...
impl<'source, S: ReadSnapshotSource + ?Sized> QueryService<'source, S> {
    ...
    pub fn current(&self) -> Result<CurrentReadModel, QueryError> {
        ...
        let attempt = snapshot
            .current_attempt
            .and_then(|id| snapshot.attempt(id))
            .cloned();
        ...
        Ok(CurrentReadModel {
            ...
            parent,
            history_position: snapshot.navigation.index.map(|index| index + 1),
            history_length: snapshot.navigation.entries.len(),
    ...
    pub fn status(&self) -> Result<StatusReadModel, QueryError> {
        ...
        Ok(StatusReadModel {
            schema_version: READ_MODEL_SCHEMA_VERSION,
            revision: snapshot.revision,
            repository_label: snapshot.repository_label,
            safety,
            current_state: snapshot.current_state,
            current_attempt: snapshot.current_attempt,
            workspace: snapshot.workspace,
            saved_states: snapshot.states.len(),
            visible_states,
    ...
    pub fn graph(&self, include_archived: bool) -> Result<GraphReadModel, QueryError> {
        layout_graph(self.snapshot()?, include_archived)
    }
    pub fn story(&self) -> Result<StoryReadModel, QueryError> {
        let snapshot = self.snapshot()?;
        let tips: BTreeSet<_> = snapshot
            .attempts
            .iter()
            .map(|attempt| attempt.tip)
            .collect();
        let entries = snapshot
            .visible_states(false)
            .filter(|state| state.kind == "nice" || state.is_starred())
            .cloned()
            .map(|state| StoryEntry {
                markers: markers_for(&snapshot, &state, &tips),
                state,
            })
            .collect();
        Ok(StoryReadModel {
    ...
    pub fn show(&self, state_id: StateId) -> Result<ShowReadModel, QueryError> {
        ...
        let state = snapshot
            .state(state_id)
            .cloned()
            .ok_or(QueryError::StateNotFound(state_id))?;
        let parent = state.logical_parent;
        let mut patch =
            self.source
                .read_diff(parent, state.id, DiffScope::Atomic, snapshot.revision)?;
        patch.canonicalize();
        Ok(ShowReadModel {
            schema_version: READ_MODEL_SCHEMA_VERSION,
            revision: snapshot.revision,
    ...
    pub fn diff(
        ...
        to: StateId,
        scope: DiffScope,
    ) -> Result<DiffReadModel, QueryError> {
        let snapshot = self.snapshot()?;
        if let Some(id) = from {
            if snapshot.state(id).is_none() {
                return Err(QueryError::StateNotFound(id));
            }
        }
        if snapshot.state(to).is_none() {
            return Err(QueryError::StateNotFound(to));
        }
...
fn safety(snapshot: &RepositorySnapshot) -> SafetyState {
    if snapshot.recovery.required {
        ...
        .warnings
        .iter()
        .any(|warning| warning.contains("reconcil"))
    {
        SafetyState::Unknown
    } else if snapshot
        .warnings
        .iter()
        .any(|warning| warning.contains("diverg"))
    {
        SafetyState::Diverged
    } else if snapshot.workspace.changes.is_dirty() {
        SafetyState::Dirty
    } else {
        SafetyState::Safe
    }
...
fn markers_for(
    ...
    tips: &BTreeSet<StateId>,
) -> BTreeSet<SemanticMarker> {
    let mut markers = BTreeSet::new();
    if snapshot.current_state == Some(state.id) {
        markers.insert(SemanticMarker::Current);
    }
    if tips.contains(&state.id) {
        markers.insert(SemanticMarker::AttemptTip);
    }
    if state.is_starred() {
        markers.insert(SemanticMarker::Starred);
    }
    if state.is_trusted() {
        markers.insert(SemanticMarker::Trusted);
    }
    if state.archived {
        markers.insert(SemanticMarker::Archived);
    }
    markers
}
...
fn layout_graph(
    ...
    let visible: BTreeMap<_, _> = snapshot
        .visible_states(include_archived)
    ...
    for state in &snapshot.states {
        ...
        let parent = match state.logical_parent {
            ...
            Some(parent) if snapshot.state(parent).is_none() => {
                return Err(QueryError::InvalidGraph {
                    state: state.id,
                    reason: format!("logical parent {parent} is absent"),
                });
            }
            _ => None,
        };
    ...
    struct Walker<'a> {
        ...
        visible: &'a BTreeMap<StateId, StateReadModel>,
        tips: &'a BTreeSet<StateId>,
        children: &'a BTreeMap<Option<StateId>, Vec<StateId>>,
        visiting: &'a mut BTreeSet<StateId>,
        visited: &'a mut BTreeSet<StateId>,
        nodes: &'a mut Vec<GraphNode>,
        edges: &'a mut Vec<GraphEdge>,
        next_lane: &'a mut usize,
    }
    impl Walker<'_> {
        fn walk(&mut self, id: StateId, depth: usize, lane: usize) -> Result<(), QueryError> {
            ...
            if !self.visiting.insert(id) {
                return Err(QueryError::InvalidGraph {
                    state: id,
                    reason: "logical-parent cycle".into(),
                });
            }
            let state = &self.visible[&id];
            self.nodes.push(GraphNode {
                state: state.clone(),
                markers: markers_for(self.snapshot, state, self.tips),
                depth,
                lane,
            });
    ...
    for (index, root) in roots.into_iter().enumerate() {
        ...
        Walker {
            ...
            nodes: &mut nodes,
            edges: &mut edges,
            next_lane: &mut next_lane,
        }
        .walk(root, 0, root_lane)?;
    }
    if visited.len() != visible.len() {
        ...
        return Err(QueryError::InvalidGraph {
            state,
            reason: "state is unreachable from every root (cycle suspected)".into(),
        });
    }
    Ok(GraphReadModel {
        schema_version: READ_MODEL_SCHEMA_VERSION,
        ...
        nodes,
        edges,
        omitted: OmissionReadModel {
            archived_states,
            incomplete: archived_states > 0,
            reasons: if archived_states > 0 {
                vec!["archived states hidden; pass include_archived to expand".into()]
            } else {
                Vec::new()
            },
        },
